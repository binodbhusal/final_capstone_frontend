...
const API_BASE_URL = 'https://motorhubapi.up.railway.app/api/motors';
...
export const createReservation = createAsyncThunk('reserve',
  async ({ reserveData, userId }, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`https://motorhubapi.up.railway.app/api/users/${userId}/reservations`, reserveData, {
        headers: {
          Authorization: `${token}`,
        },
      });
...
export const fetchReservations = createAsyncThunk('reservations',
  async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://motorhubapi.up.railway.app/api/users/${userId}/reservations`, {
        headers: {
          Authorization: `${token}`,
        },
      });
